...
function buildClient(rootDir, commit = 'local-build') {
  ...
  const raidPremiumCss = fs.readFileSync(path.join(rootDir, 'client', 'raid-premium.css'), 'utf8');
  const raidPremiumInject = `    const SUMUS_RAID_PREMIUM_CSS=${JSON.stringify(raidPremiumCss)};\n`;
  const commitInject = `    try{window.SUMUS_COMMIT=${JSON.stringify(safeCommit)};}catch(e){}\n`;
  const hotfix = dataInject + raidPremiumInject + commitInject + HOTFIX_FILES
