...
function practice(A) {
  return `<div class="page-heading"><h1>내 속도로, 연습</h1><p>범위를 고르면, 약한 단어부터 익혀요.</p></div><div class="step-label"><span>01</span>학교 선택</div>${schoolSwitch(A)}<div class="step-label"><span>02</span>연습할 범위</div>${rangePicker(A)}<div class="step-label"><span>03</span>연습 방식</div><div class="mode-grid">${Object.entries(PRACTICE_TYPES).map(([k, name]) => `<button class="mode-option ${A.mode === k ? 'selected' : ''}" data-mode="${k}" aria-pressed="${A.mode === k}">${icon(k === 'listen' ? 'sound' : k === 'mixed' ? 'sparkle' : ['spell', 'scramble', 'initial', 'vowelblank'].includes(k) ? 'records' : 'practice')}${name}${k === 'mixed' ? '<span class="grow"></span><span class="pill blue">추천</span>' : ''}</button>`).join('')}</div><div class="row between" style="margin-top:20px"><label for="practice-count" class="tiny muted">한 번에</label><select id="practice-count" style="width:122px;font-size:13px;padding:10px">${[10, 20, 30].map(n => `<option value="${n}" ${A.target === n ? 'selected' : ''}>${n}문제씩</option>`).join('')}</select></div><div class="practice-launch"><button class="btn primary full" data-action="start-practice" ${!selectedCount(A) ? 'disabled' : ''}>${A.data.active_practice ? '하던 연습 이어가기' : '연습 시작하기'} ${icon('arrow')}</button><p>틀린 단어는 잠시 뒤 다시 나와요.</p></div>`;
}
...
function ranking(A) {
  ...
  return `<div class="page-heading"><h1>함께 자라는 중</h1><p>${esc(A.data.profile.class_name)}의 연습 기록</p></div><div class="rank-intro"><div class="eyebrow">이번 주의 성장</div><h2>어제보다 조금 더.</h2><p>각자의 속도로 쌓아가는 기록이에요.</p></div><div class="segment">${[['xp', '주간 성장'], ['total', '연습량'], ['streak', '연속 학습']].map(([k, label]) => `<button data-rank-mode="${k}" class="${mode === k ? 'selected' : ''}">${label}</button>`).join('')}</div><div style="margin-top:15px">${items.length ? items.map((p, i) => `<div class="rank-row ${p.id === A.data.profile.id ? 'me' : ''}"><span class="rank-number">${p[mode] ? (items.findIndex(x => x[mode] === p[mode]) + 1) : '—'}</span>${avatar(p.avatar_key, { size: 'mini' })}<div class="grow"><strong>${esc(p.display_name)} ${p.id === A.data.profile.id ? '<span class="pill blue">나</span>' : ''}</strong><small>Lv.${p.level} · ${CHARACTERS[p.avatar_key]?.ko || '루미'}</small></div><span class="rank-score">${num(p[mode])}${unit}</span></div>`).join('') : empty('ranking', '아직 연습 기록이 없어요')}</div><p class="quiet-note">주간 성장은 최근 7일 연습 기준이에요.<br>시험 성적은 랭킹에 포함하지 않아요.</p>`;
}
function records(A) {
...
export function updateRangeSummary(A) {
  ...
  if ($('#scope-count')) $('#scope-count').textContent = `${selected.length}개 범위 · ${selectedCount(A)}개 단어`;
