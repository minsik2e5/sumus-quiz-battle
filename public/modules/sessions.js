...
export function openPracticeRecord(sessionId) {
  ...
  const rangeText = (session.range_codes || []).map(code => `${esc(code)}${session.division === 'middle' ? '과' : '번'}`).join(' · ') || '선택 범위';
