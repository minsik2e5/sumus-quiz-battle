...
function dashboard(A) {
  ...
  return `
    ...
    <section class="panel">
      ...
      ${studentTable(d.profiles.slice(0, 8))}
    </section>
  `;
}
function studentTable(list) {
  return list.length ? `<div class="table-scroll"><table><thead><tr><th>학생</th><th>반 / 학교</th><th>오늘 학습량</th><th>정답률</th><th>취약 단어</th><th>연습 횟수</th><th>상태</th></tr></thead><tbody>${list.map(p => `<tr><td><button class="table-name" data-student="${p.id}">${avatar(p.avatar_key, { size: 'mini' })}<div><strong>${esc(p.display_name)}</strong><small>Lv.${p.stats.level} · ${esc(p.username)}</small></div></button></td><td>${esc(p.class_name)}<small>${p.school}</small></td><td><strong>${num(p.stats.today_total)}</strong> 문제</td><td><div class="stat-bar"><div class="progress"><i style="width:${p.stats.accuracy}%"></i></div><span>${p.stats.accuracy}%</span></div></td><td>${p.stats.weak}개</td><td>${p.stats.practice_count}회</td><td><span class="pill ${p.active ? 'green' : ''}">${p.active ? '활성' : '일시 중지'}</span></td></tr>`).join('')}</tbody></table></div>` : empty('user', '등록된 학생이 없어요', '학생 등록으로 첫 계정을 만들어주세요.');
}
export function studentFiltered(A) {
  const q = (A.search || '').toLowerCase();
  return studentTable(A.data.profiles.filter(p => (!A.classFilter || p.class_name === A.classFilter) && `${p.display_name} ${p.username}`.toLowerCase().includes(q)));
}
function students(A) {
...
export function vocabTable(A) {
  ...
  return `<div class="panel-head"><h2>${A.school} 단어장</h2><span>${list.length}개 단어 · 원본 표현 보존</span></div><div class="table-scroll"><table class="vocab-table"><thead><tr><th>범위</th><th>영어</th><th>뜻</th><th>출처</th></tr></thead><tbody>${list.slice(0, 400).map(w => `<tr><td>${esc(w.range_code)}</td><td><strong>${esc(w.word)}</strong></td><td>${esc(w.meaning)}</td><td>${esc(w.source)}</td></tr>`).join('')}</tbody></table></div>`;
}
...
function exams(A) {
  return `<section class="panel"><div class="panel-head"><h2>실전시험 목록</h2><span>총 ${A.data.exams.length}개</span></div>${A.data.exams.length ? `<div class="table-scroll"><table><thead><tr><th>시험명 / 범위</th><th>시험 유형</th><th>대상</th><th>문제 / 시간</th><th>제출</th><th>결과 공개</th><th>운영</th></tr></thead><tbody>${A.data.exams.map(e => `<tr><td><strong>${esc(e.title)}</strong><small>${e.school} · ${esc(scope(e))}</small><small>${date(e.due_at)} 마감</small></td><td>${EXAM_TYPES[e.exam_type].label}</td><td>${esc(e.class_name)}</td><td>${e.question_count}문제 / ${Math.round(e.duration_sec / 60)}분</td><td><strong>${A.data.attempts.filter(a => a.exam_id === e.id && a.status === 'submitted').length}</strong>명</td><td><button class="text-button" data-release="${e.id}">${e.release_result ? '공개 중' : '비공개'}</button></td><td><button class="btn small" data-exam-active="${e.id}">${e.active ? '배정 중지' : '다시 배정'}</button></td></tr>`).join('')}</tbody></table></div>` : empty('exam', '첫 실전시험을 만들어보세요', '영어쓰기와 뜻쓰기도 바로 만들 수 있어요.')}</section>`;
}
function results(A) {
