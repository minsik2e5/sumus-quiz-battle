...
export function examDefaults(A) { return A.examForm ??= { title: '', class_name: A.data.profiles[0]?.class_name || (A.data.profile.active_division === 'middle' ? '중3' : '고1A'), exam_type: 'eng2mean_mc', question_count: 20, minutes: 10, passing_score: 80, max_attempts: 1, available: localDate(Date.now()), due: localDate(Date.now() + 3 * 86400000), release_result: true }; }
...
const classSelect = (A, label, name, value) => { const options = [...new Set([...divisionClasses(A), value].filter(Boolean))]; return `<label class="field"><span>${label}</span><select name="${name}" required>${options.map(c => `<option value="${esc(c)}" ${String(value) === c ? 'selected' : ''}>${esc(c)}</option>`).join('')}</select></label>`; };
