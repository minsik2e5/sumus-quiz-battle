...
export async function runReleaseCheck() {
  ...
  try {
    ...
    assert(indexSource.includes('/v1325.css?v=13.28.0') && indexSource.includes('/v1326.css?v=13.28.0'), 'V13.28 learning style layers are connected');
    ...
    assert(studentModule.includes('home-metrics') && studentModule.includes('포인트') && studentModule.includes('XP ') && sessionsModule.includes('result-reward-card'), 'V13.28 separates XP, reward points, and achievements in the student UX');
    ...
    assert(studentModule.includes('data-action="start-exam-run"') && !studentModule.includes('다음 문제마다 시간이 다시 시작돼요') && appJs.includes('examStyle: true'), 'V13.28 both exam modes share the same untimed exam-style word pool');
    ...
    assert(sessionsModule.includes("const modeLabel = testMode ? '실전시험' : '연습시험'") && !sessionsModule.includes('${timerHtml}') && sessionsModule.includes('keyboard-focus'), 'V13.28 practice and real exams share one focused untimed question screen with keyboard handling');
    assert(sessionsModule.includes('answer-impact-compact') && sessionsModule.includes('result-reward-top') && sessionsModule.includes('missingWordIds'), 'V13.28 keeps calm answer feedback and complete result review');
