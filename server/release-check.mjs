...
export async function runReleaseCheck() {
  ...
  try {
    ...
    assert(indexSource.includes('/v1325.css?v=13.29.0') && indexSource.includes('/v1326.css?v=13.29.0'), 'V13.29 learning style layers are connected');
    ...
    assert(indexHtml.includes('/app.js?v=13.29.0') && indexHtml.includes('/practice-enhancements.js?v=13.29.0') && indexHtml.includes('/v1325.css?v=13.29.0') && indexHtml.includes('/v1326.css?v=13.29.0') && indexHtml.includes('/v1327.css?v=13.29.0') && indexHtml.includes('/v1328.css?v=13.29.0') && indexHtml.includes('/v1329.css?v=13.29.0') && sw.includes('sumus-voca-v13.29.0-green-login-logo'), 'V13.29 cache versions are active');
    ...
    assert(v1326Css.includes('.home-focus-v1326') && v1326Css.includes('.rank-filter-bar') && v1326Css.includes('.pwa-install-hint'), 'V13.29 home, ranking, and PWA polish styles are loaded');
    assert(v1327Css.includes('.home-pet-hero') && v1327Css.includes('.record-summary-v1327') && v1327Css.includes('.auth-card-v1327'), 'V13.29 student home, records, and login polish styles are loaded');
    assert(v1328Css.includes('.home-pet-art .avatar') && v1328Css.includes('.exam-question-area') && v1328Css.includes('.teacher-division-switch'), 'V13.29 mobile ratio and teacher division switch styles are loaded');
    assert(v1329Css.includes('.auth-v1327') && v1329Css.includes('box-sizing:border-box') && indexSource.includes('/sumus-logo-green.svg'), 'V13.29 green login shell and logo are loaded');
    assert(studentModule.includes('home-pet-hero') && studentModule.includes('home-week-days') && studentModule.includes('data-rank-scope-select') && !studentModule.includes('같이 올라가면 더 재밌다.'), 'V13.29 home is pet-and-growth focused and ranking filters are compact');
    assert(appJs.includes('rankScopeSelect') && appJs.includes('missingCount') && appJs.includes('session.word_ids'), 'V13.29 compact rank filters and complete review flow are wired');

    assert(studentModule.includes('home-metrics') && studentModule.includes('포인트') && studentModule.includes('XP ') && sessionsModule.includes('result-reward-card'), 'V13.29 separates XP, reward points, and achievements in the student UX');
    ...
    assert(studentModule.includes('data-action="start-exam-run"') && !studentModule.includes('다음 문제마다 시간이 다시 시작돼요') && appJs.includes('examStyle: true'), 'V13.29 both exam modes share the same untimed exam-style word pool');
    ...
    assert(sessionsModule.includes("const modeLabel = testMode ? '실전시험' : '연습시험'") && !sessionsModule.includes('${timerHtml}') && sessionsModule.includes('keyboard-focus'), 'V13.29 practice and real exams share one focused untimed question screen with keyboard handling');
    assert(sessionsModule.includes('answer-impact-compact') && sessionsModule.includes('result-reward-top') && sessionsModule.includes('missingWordIds'), 'V13.29 keeps calm answer feedback and complete result review');
