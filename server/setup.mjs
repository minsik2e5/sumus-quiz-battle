...
const password = process.env.ADMIN_PASSWORD || await rl.question('교사 비밀번호 (8자 이상): ');
rl.close();
if (!/^[a-z0-9_.-]{3,40}$/.test(username) || password.length < 8) { console.error('아이디는 영문·숫자 3~40자, 비밀번호는 8자 이상 필요합니다.'); repo.close(); process.exit(1); }
